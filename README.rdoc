...
The application is rails 4 using PG for the database. 
...
The form displayed error if the message is empty
Yes, functionality can be seen at: http://untwitter.herokuapp.com/

The form displayed error if the message is more than 150 characters:
...
The page displayed a flash message if form submitted without errors
Yes, functionality can be seen at: http://untwitter.herokuapp.com/
...
Each item in the timeline contains a delete button
Yes: http://untwitter.herokuapp.com/

Message can contain newlines
...
Feature test: unsuccessful submission are passing - Done
